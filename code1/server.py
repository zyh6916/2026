...
from utils.sign import deserialize_keys, sign_message
...
def aggregatie_weights(rec,recv_list,weights_client,total_sum,batch_num,id_list,args,enc_tools = {},rep_num = []):
    ...
    # # 加载客户端的公私钥
    # with open('clients_keys.pkl', 'rb') as f:
    #     loaded_clients_keys = pickle.load(f)

    # # 恢复公私钥
    # for client_id, keys in loaded_clients_keys.items():
    #     private_key_bytes = keys['private_key']
    #     public_key_bytes = keys['public_key']
    ...
    #     # 替换恢复后的私钥和公钥
    #     loaded_clients_keys[client_id]['private_key'] = private_key
    #     loaded_clients_keys[client_id]['public_key'] = public_key
    ...
    # 恢复公私钥
    for client_id, keys in loaded_clients_keys.items():
        sk_bytes = keys['private_key']
        pk_bytes = keys['public_key']
        
        # 恢复私钥和公钥
        private_key, public_key = deserialize_keys(sk_bytes, pk_bytes)
        ...
        if args.enc:
            ...
            if args.algorithm == 'ckks':
                ...
                if  args.isSpars == 'topk':     
                    ...
                    private_key = loaded_clients_keys[c_id]['private_key']
                    public_key = loaded_clients_keys[c_id]['public_key']
                    
                    
                    # 为该客户端签名
                    signature, msg_hash = sign_message(private_key, c_id, cipher, mask)
                    
                    # 收集签名数据
                    signatures.append(signature)
                    messages.append(msg_hash)
    ...
    # 聚合公钥
    aggregated_public_key = G1Element()
    ...
    try:
        # 验证聚合签名
        # msg_hashes = [sha256(message).digest() for message in messages]
        if not AugSchemeMPL.aggregate_verify(public_keys, messages, aggregated_signature):
            valid = False
            print("验证失败: 批量验证不通过")
    ...
    # 输出验证结果
    print(f"Verification Time: {verification_end_time - verification_start_time:.6f} seconds")
    if valid:
        print("所有签名验证通过")
...
def server_process(args,kwargs_IPC,total_sum,batch_num,train_weights,test_weights,server_test_sets,kwargs):
    ...
    # 🟥 初始化 weights_client 为固定的训练权重
    # weights_client = [0.057786222644465556, 0.37075187092687967, 0.16217477540543693, 0.11243895528109739, 0.020884377552210943, 0.020884377552210943, 0.007367035018417588, 0.24771238561928097]
    # weights_client =[0.12042, 0.09948, 0.12594, 0.16376, 0.09666, 0.09666, 0.16654, 0.13054]

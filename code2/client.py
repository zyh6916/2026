...
# --- [新增：包级抵消掩码生成逻辑] ---
def generate_batch_mask(my_rank, selected_ids, batch_idx, seed_map, size):
    mask = np.zeros(size)
    for other_id in selected_ids:
        if my_rank == other_id: continue
        pair = tuple(sorted((my_rank, other_id)))
        seed = seed_map.get(pair, sum(pair)) + batch_idx
        rng = np.random.RandomState(seed)
        r_ij = rng.randn(size) * 0.001
        if my_rank < other_id: mask += r_ij
        else: mask -= r_ij
    return mask
...
def params_tolist(model):
    model.to('cpu')
    local_state = model.state_dict()
    params_list = []; layer_shape = {}; params_num = {}
    for key in local_state.keys():
        layer_shape[key] = local_state[key].shape
        params_num[key] = int(np.prod(local_state[key].shape))
        layer_params = local_state[key].reshape(params_num[key]).tolist()
        params_list.append(layer_params)
    flat_params = [b for a in params_list for b in a]
    return flat_params, params_num, layer_shape

def params_tomodel(model,global_list,params_num,layer_shape,args,params_list):
    update_state = OrderedDict(); model.to('cpu'); idx_cnt = 0      
    if args.isSpars in ['topk', 'randk']:
        for key in model.state_dict().keys():
            layer_size = int(params_num[key])
            tmp = global_list[idx_cnt : idx_cnt + layer_size]
            for i in range(len(tmp)):
                if tmp[i] == 0 and (i == len(tmp)-1 or (i+1 < len(tmp) and tmp[i+1]==0)):
                    tmp[i] = params_list[idx_cnt + i]
...
def minHash(rank,random_R,global_list,params_list, args, quan_thres = 0.05):
    mat = np.concatenate((np.array(global_list).reshape(-1,1),np.array(params_list).reshape(-1,1)),axis=1)
    quan_matrix = min_lsh.quan_params(mat,quan_thres)
    sim_mat = min_lsh.sigMatrixGen(quan_matrix,random_R, args.sim_len)
    return min_lsh.apply_rr((sim_mat[:,1]).tolist(), getattr(args, 'rr_p', 0.8))
...
def client_process(rank, args, model, device,dataset, test_dataset, kwargs,kwargs_IPC,train_weights):
    torch.manual_seed(args.seed + rank)
    queue = kwargs_IPC['queues'][rank]; e = kwargs_IPC['e']; lock = kwargs_IPC['lock']
    pipe = kwargs_IPC['client_pipes'][rank][0]; flag = kwargs_IPC['flag']
    e_server = kwargs_IPC['e_server']; hash_queue = kwargs_IPC['hash_queue']
    acc_pipe = kwargs_IPC['send_pipes'][rank][0]; pairwise_seeds = kwargs_IPC.get('pairwise_seeds', {})
    if args.isSelection: random_R = kwargs_IPC['random_R']
    
    train_loader = torch.utils.data.DataLoader(dataset, **kwargs)
    test_loader = torch.utils.data.DataLoader(test_dataset, **kwargs)
    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    epoch = 0; self_flag = True; self_weight = train_weights[rank]
    ...
    while not flag.value:
        ...
        if self_flag:    
            if args.enc and args.isSpars == 'topk':
                param_batches = [params_list[i:i + args.enc_batch_size] for i in range(0, len(params_list), args.enc_batch_size)]
                batch_norms = [np.linalg.norm(b) for b in param_batches]
                k_count = max(1, int(len(batch_norms) * args.topk))
                local_batch_mask = np.zeros(len(batch_norms), dtype=bool)
                local_batch_mask[np.argsort(batch_norms)[-k_count:]] = True
                
                # 第一阶段：握手
                pipe.send(["INDEX_PROPOSAL", rank, local_batch_mask])
                sync_data = pipe.recv()
                union_idx = sync_data['union_idx']; selected_ids = sync_data['selected_clients']
                
                ckks_ctx = ts.context_from(open(os.path.join(args.data_dir + 'context_params'), "rb").read())
                cipher_list = []
                for b_idx in union_idx:
                    raw_b = np.array(param_batches[b_idx]) if b_idx < len(param_batches) else np.zeros(args.enc_batch_size)
                    if len(raw_b) < args.enc_batch_size: raw_b = np.pad(raw_b, (0, args.enc_batch_size - len(raw_b)))
                    # 应用掩码
                    p_mask = generate_batch_mask(rank, selected_ids, b_idx, pairwise_seeds, len(raw_b))
                    cipher_list.append(ts.ckks_vector(ckks_ctx, raw_b + p_mask).serialize())
                pipe.send(["CIPHER_DATA", rank, union_idx, cipher_list])
            else:
                pipe.send([rank, params_list])
        ...
        e.wait()
        global_list = queue.get()
        involved_frac, global_weights_cipher = global_list[0], global_list[1]
        if args.enc:
            ckks_ctx = ts.context_from(open(os.path.join(args.data_dir + 'context_params'), "rb").read())
            global_weights = ckks_dec(global_weights_cipher, ckks_ctx, ckks_ctx.secret_key(), args.isBatch, [], involved_frac, args.enc_batch_size).tolist()
            global_weights = global_weights[:sum(params_num.values())]
        else:
            global_weights = (np.array(global_weights_cipher) / involved_frac).tolist()
        ...
        if args.isSelection:
            client_hash = minHash(rank, random_R, global_weights, params_list, args)
            hash_queue.put([rank,client_hash]); e_server.wait()
            # 接收广播名单
            selection_msg = pipe.recv()
            if selection_msg[0] == "NEXT_ROUND_INFO":
                clients_share, clients_weights = selection_msg[1], selection_msg[2]
                self_flag = (rank in clients_share)
                if self_flag: self_weight = clients_weights[clients_share.index(rank)]
        epoch += 1
    return

def train_epoch(epoch, args, model, device, data_loader, optimizer, rank):
    model.to(device); model.train(); loss_fn = torch.nn.CrossEntropyLoss()
    for batch_idx, (data, target) in enumerate(data_loader):
        optimizer.zero_grad()
        loss = loss_fn(model(data.to(device)), target.to(device))
        loss.backward(); optimizer.step()

def test_epoch(model, device, data_loader):
    model.to(device); model.eval(); correct = 0; test_loss = 0
    with torch.no_grad():
        for data, target in data_loader:
            output = model(data.to(device))
            test_loss += F.cross_entropy(output, target.to(device), reduction='sum')
            correct += output.max(1)[1].eq(target.to(device)).sum().item()
    return round((correct/len(data_loader.dataset))*100, 2), round(float(test_loss/len(data_loader.dataset)), 3)

def dec_params(cipher_list,sum_masks, enc_tools,args):
    ckks_ctx = ts.context_from(open(os.path.join(args.data_dir + 'context_params'), "rb").read())
    return ckks_dec(cipher_list,ckks_ctx,ckks_ctx.secret_key(),args.isBatch,[],sum_masks,args.enc_batch_size)
...
#                             help='datasets: MNIST, FashionMNIST, CIFAR10, CIFAR100') # -----------------------数据集
#     parser.add_argument('--batch-size', type=int, default=64, metavar='N',
#                         help='input batch size for training (default: 64)')
#     parser.add_argument('--epochs', type=int, default=100 , metavar='N',
#                         help='number of epochs to train (default: 10)')# -----------------------轮数
#     parser.add_argument('--lr', type=float, default=0.001, metavar='LR',
#                         help='learning rate (default: 0.001)')
#     parser.add_argument('--weighted',type=bool,default=False)  # -----------------------权重
...
#                         help='Client selection or not')       # -----------------------客户端选择
...
#                         help='sparfication fraction')             # -----------------------比例
                  
#     # encryption paramsy
#     parser.add_argument('--isBatch',type=bool,default=True,
#                         help='Batch HE or not')                 # -----------------------批处理
...
#                         help='HE algorithm: paillier,bfv, ckks') # -----------------------加密方式
#     parser.add_argument('--quan_bits',type=int,default=16,
#                         help='quantification bits')
#     parser.add_argument('--enc_batch_size',type=int,default=4096,  # -----------------------80/4096
...
def arg_parse():
    ...
    # --- [新增：隐私保护参数] ---
    parser.add_argument('--rr_p', type=float, default=0.8,
                        help='Randomized Response probability (Privacy Budget)')
...
def main():
    ...
    # 自动化清理旧的名单文件
    selected_path = os.path.join(args.data_dir, args.dataset + 'selected')
    ...
    for i in range(n_clients):
        for j in range(i + 1, n_clients):
            # 为系统中的每一对 Silo 生成一个唯一的随机种子
            # 保证 key 为 (min_id, max_id) 格式
            pairwise_seeds[(i, j)] = np.random.randint(0, 1000000)
    
    # 注入到 IPC 字典中，供 client.py 的 generate_aligned_mask 调用
    kwargs_IPC['pairwise_seeds'] = pairwise_seeds
    ...
    logging("Basic information: device {}, learning rate {}, num clients {}, epochs {}, noniid_method {},\
            isEnc {}, isBatch {}, sparsification {}, client selection {}, topk {}, enc_batch_size {}, rr_p {}".format(
        device, args.lr, args.n_clients, args.epochs, args.noniid_method, args.enc, args.isBatch, 
        args.isSpars, args.isSelection, args.topk, args.enc_batch_size, args.rr_p), args)

...
def jaccard_affinity_propagation(matrix):
    # Calculate the similarity matrix (negative distances for affinity propagation)
    distance_matrix = jaccard_distance_matrix(matrix)
    ...
    cluster_indices = [np.where(ap.labels_ == i)[0] for i in range(len(ap.cluster_centers_indices_))]
